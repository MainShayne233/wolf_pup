...
(function() {
  ...
  this.log_dealer = function(latex, step) {
    var base, box, val;
    step += 3;
    base = '';
    if (latex[step + 1] === '_') {
  ...
  this.ln_dealer = function(latex, step) {
    step += 2;
  ...
  this.sumprod_dealer = function(latex, step) {
    var arg, box, f, lims, ref, val;
    if ((latex.slice(step + 1, +(step + 3) + 1 || 9e9).join('')) === 'sum') {
      f = 'sum';
      step += 4;
    } else if ((latex.slice(step + 1, +(step + 4) + 1 || 9e9).join('')) === 'prod') {
      f = 'product';
      step += 5;
    }
    lims = '';
  ...
  this.tau_dealer = function(latex, step) {
    var val;
    step += 3;
    val = '%28tau%29';
    return box_boy(val, step);
  ...
  this.theta_dealer = function(latex, step) {
    step += 5;
  ...
  this.is_log = function(latex, step) {
    return (latex.slice(step + 1, +(step + 3) + 1 || 9e9).join('')) === 'log';
  };

  this.is_ln = function(latex, step) {
    return (latex.slice(step + 1, +(step + 2) + 1 || 9e9).join('')) === 'ln';
  };
  ...
  this.is_sumprod = function(latex, step) {
    return (latex.slice(step + 1, +(step + 3) + 1 || 9e9).join('')) === 'sum' || (latex.slice(step + 1, +(step + 4) + 1 || 9e9).join('')) === 'prod';
  };

  this.is_tau = function(latex, step) {
    return (latex.slice(step + 1, +(step + 3) + 1 || 9e9).join('')) === 'tau';
  };
  ...
  this.is_theta = function(latex, step) {
    return (latex.slice(step + 1, +(step + 5) + 1 || 9e9).join('')) === 'theta';
  };
  ...
  this.ups_guy = function(latex, step) {
    ...
    return alert("Something went wrong, sorry. It'd be very appreciated if you could send an email via the contact page with this code: " + latex);
  };
  ...
  this.wolf_write = function(latex) {
    return $('#input').mathquill('write', latex);
  };
